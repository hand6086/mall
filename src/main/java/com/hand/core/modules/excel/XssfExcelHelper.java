...
import com.hand.core.util.ReflectUtil;
...
public class XssfExcelHelper extends ExcelHelper {
    ...
    public <T> List<T> readExcel(Class<T> clazz, String[] fieldNames,
        ...
        //如果有标题，校验标题长度是否相等，且不能为空
        if(start == 1){
        	XSSFRow xssfRow = sheet.getRow(0);
        	if(xssfRow == null) throw new Exception("请使用正确模板进行导入！");
        	if(xssfRow.getLastCellNum() != fieldNames.length) throw new Exception("请使用正确模板进行导入！");
        }
        ...
        for (int i = start; i <= sheet.getLastRowNum(); i++) {
            ...
            for (int j = 0; j < fieldNames.length; j++) {
                ...
                }else if(isDoubleType(clazz, fieldName)){
