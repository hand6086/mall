 ...
 <div id="forumBodyFirstDiv" data-options="region:'south'" collapsible="false"  border="false" style="width:100%;height:100%;padding:0px;">
	    <div class="topic-bg" >
			...
			<ul class="tbui_aside_float_bar">
				<li class="float_bar_li"><a class="float_bar_li_a3" onclick="openAddClueWin('${param.clueId}')"></a></li>
	...
	<!-- 弹出新增线索跟进----------begin -->
	<div id="addClueWin" class="easyui-window"
		style="width: 100%; height: 100%" data-options="iconCls:'icon-save'">
		<iframe src="" id="addClueWinFram" name="addClueWinFram"
	...
	//新增线索跟进弹窗
	$addClueWin = $("#addClueWin");
	$addClueWinFram = $("#addClueWinFram");
			...
			pageFlag : false
				...
				if (data.success) {
					...
					for(var i = 0; i < data.rows.length; i++){
						var row = data.rows[i];
						var html = '<div class="small_picture"><a  href="'+'#bigImg'+'"><img  src="'+row.attachmentPath+'"  class="img200"/></a></div>'
						$('#div-' + row.id).append($(html));
						console.log($('#div-' + row.id));
					}
				} else {
	...
	$.post('${AppContext.ctxPath}/action/portal/flowUp/queryByExamplePage',
			{sort:'created',order:'desc',clueId:'${param.clueId}',pageFlag:false},
			function(data){
				if(data.success){
					...
					for(var i=0;i<row.length;i++){
						...
						html=html+'<li class="thread-item">'
							+"<div><p class='forumheader'>提出人："+row[i].emp+"&nbsp;&nbsp;提出时间："+row[i].dt+"</p></div>"
	...
	function forumListRefresh(){
		...
		$.post('${AppContext.ctxPath}/action/portal/flowUp/queryByExamplePage',
				{sort:'created',order:'desc',clueId:'${param.clueId}',pageFlag:false,searchParam:searchParam,searchValue:searchValue},
				function(data){
					if(data.success){
						totalPage = data.total
						var row=data.rows;
	...
	/* function addFlowUp(){
		...
		$.post(
			'${AppContext.ctxPath}/action/portal/flowUp/insert',
			{
				optyId : optyId,
				dt : row.dt,
				description : row.description, //线索简要	
				type : row.type,
				orgId : appCtx.orgId,
		...
		function openAddClueWin(clueId) {
			if (!$addClueWinFram.attr("src")) {
				$addClueWinFram.attr("src",
						"${AppContext.ctxPath}/portal/clue/NewClueFollowUp.jsp?clueId="+clueId);
				$addClueWinFram[0].onload = function() {
					//$addOptyWinFram[0].contentWindow.init(newAccntId);//等待窗口加载完毕
				}
			} else {
				//$addOptyWinFram[0].contentWindow.init(newAccntId);
			}
			$addClueWin.window("open");
		}
		
		function closeAddClueWin() {
			$addClueWin.window("close");
