						   		...
						   		<fieldset>
									...
									<table class="table-block">
										<tr>
											...
											<td class="td-label"><label>商品秒杀价 :</label></td>
											<td class="td-input">
										...
										<tr>
											...
											<td class="td-input">
												<input class="easyui-validatebox" name="lmtAmt" type="text" id="lmtAmt" size="25" required="required"/>
											</td>
				...
				<table id="goods-table" fit="true">
					<thead>
						<tr>
							...
							<th field="promotionPrice" width="120px" editor="{type:'numberbox',options:{min:0,precision:2,required:true}}">秒杀价</th>
...
<script type="text/javascript">
	...
	function save(){
		...
		if(promotionPrice =='' || promotionPrice == null){
			 $.messager.alert("错误", "秒杀价不能为空", "error");
			 return false;
		}
						   ...
						   if (!data.success) {
							   window.parent.operationtip(data.result,'error');
								...
								$.messager.confirm("操作提示", '促销活动已提交,是否要跳转到已报名列表？', function(data) {
									if(data){
										window.top.quickEnter('秒杀已报名列表','秒杀已报名列表','BRAND_CAMPAIGN_MANAGEMENT','${AppContext.ctxPath}/portal/brandPromotion/brandSeckillCampaignApplyList.jsp');
									}
								});
							}
						});
