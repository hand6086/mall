...
<body>
	<div class="easyui-layout" data-options="fit:true" border="false">
		<div data-options="region:'north',split:true" title="" border="false" style="width:100%;height:100%;padding:0px;" fit="true">
			...
			<table id="campStackLine-table" width="98%" height="370px" singleSelect="true" rownumbers="true" pagination="true">
				<thead>
					<tr>
						<th field="picUrl" width="160px" formatter="imageFormatter">图片</th>
						<th field="prodName" width="20%" editor="{type:'picklistbox',options:{title:'选择商品',
						...
						<th field="prodCode" width="20%">商品编码</th>
	...
	<script type="text/javascript">
		...
		$linDatagrid = $('#campStackLine-table');
		...
		$camDatagrid.hdatagrid({
			...
			subGridIds: ['campStackLine-table'],
			...
			onChangeSelectedItem:function(row,that){
 			...
 			onAddMoreButton:function(that,addButtonFun){
				addButtonFun(92,'<a href="javascript:void(0)" id="btn_camadd" class="easyui-linkbutton" onClick="Invalid()" iconCls="icon-undo" plain="true">失效</a>',that);
 				addButtonFun(92,'<div style = "width:78px;display:inline-block"> <form id="form" method="post" enctype="multipart/form-data"> <a href="javascript:;" class="file easyui-linkbutton fileCampImg" iconCls="icon-uploadImg" plain="true">上传图片 <input type="hidden" id="campId" name="id"/>'+"<input type='hidden' name='method' value='upload'/><input type='file' id='file1' name='myfiles'/></a></form></div>",that);
			}, 
		...
		$linDatagrid.hdatagrid({
			...
			pageSize: 10,//每页显示的记录条数，默认为20 
		    pageList: [10,30,50],//可以设置每页记录条数的列表 
			url:'${AppContext.ctxPath}/action/portal/camStack/queryByExamplePage',
		    ...
		    onInsertOrUpdateAction:function(row,successFun,errFun,that){
				...
				$.post("${AppContext.ctxPath}/action/portal/camStack/upsert",row, 
				function(data) {
					if (!data.success) {
						errFun(data.result,that);
					}else{
			...
			onDeleteAction : function(row,successFun,errFun,that){
				$.messager.confirm("操作提示", '是否确定删除信息？', function(data) {
					if(data){
						$.post('${AppContext.ctxPath}/action/portal/camStack/deleteById', {
								id : row.id
						}, function(data) {
								if (!data.success) {
									errFun(data.result,that);
