...
<body>
			...
			<table id="campaign-table" width="98%" height="370px" singleSelect="true" rownumbers="true" pagination="true">
				<thead>
					<tr>
						...
						<th field="code" width="160px" >编码</th>
						...
						<th field="shareTheme" width="100px" >分享标题</th>
						<th field="URL" width="130px" >URL</th>
						<th field="shareNotes" width="150px" >分享说明</th>
						<th field="type" width="130px" lov="TB_O2O_SHEET_TYPE">类型</th>
						
				   </tr>
				</thead>
			</table>
			
			<table id="campCoupLine-table" width="98%" height="370px" singleSelect="true" rownumbers="true" pagination="true">
				<thead>
					<tr>
						...
						<th field="couponCode" width="180px">优惠券编码</th>
						<th field="displaySeq" width="120px" editor="{type:'numberbox',options:{
																					min:0,
																					precision:0}}">显示顺序</th>
						<th field="status" width="160px" lov="TB_O2O_STATUS" editor="{type:'combobox',options:{valueField:'val',
																			textField:'name',
																			data : getLovArray('TB_O2O_STATUS'),
																			editable:false}}">状态</th>
	...
	<script type="text/javascript">
		...
		$linDatagrid = $('#campCoupLine-table');
		...
		$linDatagrid.hdatagrid({
			...
			pageSize: 10,//每页显示的记录条数，默认为20 
		    pageList: [10,30,50],//可以设置每页记录条数的列表 
			url:'${AppContext.ctxPath}/action/portal/camCoupon/queryByExamplePage',
		    ...
		    onInsertOrUpdateAction:function(row,successFun,errFun,that){
				...
				$.post("${AppContext.ctxPath}/action/portal/camCoupon/upsert",row, 
				function(data) {
					if (!data.success) {
						errFun(data.result,that);
					}else{
			...
			onDeleteAction : function(row,successFun,errFun,that){
				$.messager.confirm("操作提示", '是否确定删除信息？', function(data) {
					if(data){
						$.post('${AppContext.ctxPath}/action/portal/camCoupon/deleteById', {
								id : row.id
						}, function(data) {
								if (!data.success) {
									errFun(data.result,that);
