...
<body>
	...
	function getdata(){
		var data = getLovArray('TB_O2O_CAM_LINE_TYPE');
		
   		for (var i = data.length - 1; i >=0; i--) {
			if (data[i].val != 'Picture' && data[i].val != 'Product') {
				data.splice(i,1);
			}
		}
		return data;
	...
	$('#prodName').picklist({
		...
		tableUrl:'${AppContext.ctxPath}/portal/picklist/GoodsPicklist.jsp'
	});
	...
	$('#couponName').picklist({
		...
		initParam:function(selectRow){
			return {source:'fast shopping',type:'Coupon'}
		},
	...
	$('#promotionName').picklist({
		...
		initParam:function(selectRow){
			return {source:'fast shopping',type:'Second Kill'};
		},
		tableUrl:'${AppContext.ctxPath}/portal/picklist/Tbo2oSecKillPickList.jsp'
	});
	...
	function save(){
		...
		$campaignLineForm.ajaxSubmit({
			url : '${AppContext.ctxPath}/action/portal/campaignLine/camLinesInsert',
			type : 'post',
			dataType :'json',
			data: {attr1:campaignId},
