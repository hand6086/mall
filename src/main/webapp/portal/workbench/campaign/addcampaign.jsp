...
.td-content input{
							...
							<form id="campaignForm" method="post"> 
								<table class="table-block" >							
									<tr>
										...
										<td class="td-content">
											<input  class="easyui-combobox" name="type" type="text" id="type"  disabled></textarea>
										</td>
									...
									<tr>
										...
										<td class="td-content">
											<a href="javascript:void(0)" id="selectCityBtn" onclick="selectCity()">选择城市</a>
										</td>
	...
	function save() {
			if(records.length == 0){
				window.parent.operationtip('请选择城市', 'info');
				return;
			}
			$(".save").linkbutton('disable');
	 		var options = {
					url : '${AppContext.ctxPath}/action/portal/campaign/camsInsert',
					type : 'post',
					dataType :'json',
					data:{attr1:records.join(","),type:"${param.type}"},
					...
					success : function(data) {
						if (data.success) {
							window.parent.closeAddWin("ok",data.newRowIds,data.newRowType);
						} else {
							$(".save").linkbutton('enable');
