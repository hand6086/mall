						   ...
						   <td>
						   		<fieldset>
									...
									<table class="table-block">
										...
										<tr>
											<td class="td-label"><label>促销特价 :</label></td>
											<td class="td-input">
												<input class="easyui-numberbox" name="promotionPrice" type="text" id="promotionPrice" size="25" data-options="min:0,precision:2" required="required" />
											</td>
				...
				<table id="goods-table" fit="true">
					<thead>
						<tr>
							...
							<th field="purchasedQty" width="110px"
								editor="{type:'numberbox',options:{min:0,precision:0,required:true}}">已售总量</th>
...
<script type="text/javascript">
	...
	function saveGoods(){
		...
		if(promotionPrice =='' || promotionPrice == null){
			 $.messager.alert("错误", "促销价不能为空", "error");
			 return false;
		}
		...
		$.post("${AppContext.ctxPath}/action/portal/promotionGoods/insert",{
			...
			totalQty       :   totalQty,
			purchasedQty   :   '0',
			brandDiscountType  : 'Bargain_Price'
		},function(data){
            isClick=true;
			if(data.success){
	...
	//返回按钮
	function goBack(){
		window.location.href = '${AppContext.ctxPath}/portal/brandShop/brandBargainPriceProductList.jsp?headId=' + headId + "&startTime=" + startTime + "&endTime=" + endTime;
	}
	...
	function Invalid(){
		...
		$.messager.confirm("操作提示", warningMessage, function(data) {
			if(data){
				row.brandDiscountType = 'Bargain_Price';
				$.post('${AppContext.ctxPath}/action/portal/promotionGoods/update', row, function(data) {
					if (!data.success) {
