...
<body class="easyui-layout" data-options="fit:true" border="false">
   <div data-options="region:'north',split:true" border="false" style="width:100%;" fit="true">
	...
	<table id="activityResult-table" style="height:50%;width:100%" keyFilterDiv="keyFilter">
		<thead>
			<tr>
				<th field="id" width="100px" >编号</th>
				<th field="created" width="100px" >创建时间</th>
				<th field="Name" width="100px" defaultSearch="false" editor="{type:'validatebox',options:{required:false}}">名称</th>
				<th field="Value" width="100px" defaultSearch="false" editor="{type:'validatebox',options:{required:false}}">值</th>
	...
	<script type="text/javascript">
		...
		$datagrid.hdatagrid({
			...
			buttonBatchEdit : true,
				...
				$.post('${AppContext.ctxPath}/action/portal/activityResult/upsert', row, function(data) {
					if (!data.success) {
			...
			onDeleteAction:function(row,successFun,errFun,that) {
				$.messager.confirm("操作提示", '是否确定删除信息？', function(data) {
					if(data){
						$.post('${AppContext.ctxPath}/action/portal/activityResult/deleteById', {
								id : row.id
						}, function(data) {
							if (!data.success) {
								errFun(data.result,that);
		...
		function ActivityResultSubmit(){
			var row = $datagrid.datagrid('getSelected');
			if(row.ActivityResultStatus == '新建'||row.ActivityResultStatus == '已拒绝'){
				$.post('${AppContext.ctxPath}/action/portal/ActivityResult/ActivityResultSubmit', 
				{
					id	: row.id,
					ActivityResultStatus : '已提交'
			    },
				function(data){
			    	if(!data.success){
				...
				window.parent.operationtip('该线索状态为'+row.ActivityResultStatus+'，无法提交！', 'error');
			}
		}
