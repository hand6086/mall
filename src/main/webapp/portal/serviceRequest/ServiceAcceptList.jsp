...
<body>
	<div class="easyui-layout" data-options="fit:true" border="false">
		<div data-options="region:'north'" border="false" style="width: 100%; height: 50%; padding: 0px;">
			<table id="serviceAccept-table">
				<thead>
					<tr>
						<th field="serviceCode" width="150px">单号</th>
						<th field="createdByName" width="150px">创建人</th>
						<th field="status" lov="SERVICE_STATUS" width="100px">状态</th>
						<th field="acctName" width="150px" editor="{type:'picklistbox',options:{title:'选择客户',
															completeFun:accntPicklistComplete,
															required:true,
															tableUrl:'${AppContext.ctxPath}/portal/serviceRequest/picklist/AccntPicklist.jsp'}}">客户</th>
						<th field="contacts" width="150px" editor="{type:'validatebox'}">现场联系人</th>
						<th field="phoneNum" width="150px" editor="{type:'validatebox'}">现场联系电话</th>
						<th field="type" width="110px" lov="SERVICE_TYPE" editor="{type:'combobox', options:{valueField:'val',
																			textField:'name',
																			method:'get',
																			required:true,
																			data:getLovArray('SERVICE_TYPE'),
																			editable:false}}">维修内容</th>
						<th field="isSupplement" width="130px" lov="SERVICE_IS_SUPPLEMENT" editor="{type:'combobox', options:{valueField:'val',
																			textField:'name',
																			method:'get',
																			required:true,
																			data:getLovArray('SERVICE_IS_SUPPLEMENT'),
																			editable:false}}">是否需要增补材料</th>
						<th field="returnVisit" width="250px" formatter="tooltipFormatter" editor="{type:'validatebox'}">回访记录</th>
					</tr>
				</thead>
			</table>
		</div>
  ...
  <!-- ------------------------上传附件弹出框：开始----------------------------  -->
		...
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'center',border:false" style="padding:10px;background:#fff;border:1px solid #ccc;">
				<form method="post" class="uploadAttachmentForm" style="text-align:center;margin-top: 30px;" enctype="multipart/form-data" >
					...
					<br/><br/>类型：
					<input id="documentType" name="documentType" class="easyui-combobox" data-options="valueField:'name',
																			textField:'name',
																			width:'200px',
									         							  	height:'30px',
																			method:'get',
	...
	$serviceAcceptDatagrid = $('#serviceAccept-table');
	$attachmentDatagrid = $('#attachment-table');
	getLovOfList("[SERVICE_TYPE,SERVICE_IS_SUPPLEMENT]");
	
	/* --------------------售后服务受理单：开始------------------------------ */
	$serviceAcceptDatagrid.hdatagrid({
		updateable:true,
		toolbarShow:true,
		buttonSearch:true,
		buttonNew:true,
		...
		height:'100%',
		title:'售后服务受理单',
		//url : '${AppContext.ctxPath}/action/portal/serviceAccept/queryListPage',
		url : '${AppContext.ctxPath}/action/portal/serviceAccept/queryByExamplePage',
		...
		buttonBatchEdit : true,
					onBatchUpdateAction : function(rows, successFun, errFun, that){
						...
						$.ajax({
							url: "${AppContext.ctxPath}/action/portal/serviceAccept/batchUpdate",
							type:"post",
			...
			$.post('${AppContext.ctxPath}/action/portal/serviceAccept/upsert',row , function(data) {
				if (!data.success) {
	...
	function accntPicklistComplete(selectRow){
		var currentRow = $serviceAcceptDatagrid.hdatagrid('getCurrentRow');
	 ...
	 * 提交受理单
	 */
	function serviceAcceptSubmit(){
		if(true){
			$.messager.alert('提示',"功能待完善。");
			return;
		}
		var row = $serviceAcceptDatagrid.hdatagrid('getCurrentRow');
		$.post('${AppContext.ctxPath}/action/portal/serviceAccept/submit', row, function(data){
			if(data.success){
				$serviceAcceptDatagrid.datagrid('reload');
			}else{
