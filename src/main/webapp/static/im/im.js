;(function(RCS){
	...
	var updateMessage = function(message){
		if (message.messageType == 'ReadReceiptMessage' || message.messageType == 'TypingStatusMessage') {
		...
		if (message.messageDirection != 1 && supportNot) {
			...
			jQuery.post("/action/portal/rcstoken/userInfo",{userId:message.targetId},function(date){
				newMessage["senderUserName"] = date.name;
				newMessage["senderPortraitUri"] = date.portrait;
			});
		...
		newMessage["myPortrait"] = myPortrait;
	...
	var loadHisMessages = function(){
		var callbacks = function(list,hasMsg){
			...
			messageList.myPortrait = myPortrait;
			jQuery.ajaxSettings.async = false;
			jQuery.post("/action/portal/rcstoken/userInfo",{userId:conversation.id},function(date){
				messageList.senderName = date.name;
				messageList.senderPortrait = date.portrait;
			});
			jQuery.ajaxSettings.async = true;
		  	...
		  	onSuccess: function(list) {
		    	...
		    	for (var i = 0; i < list.length; i++) {
		    		if (list[i].conversationType == RongIMLib.ConversationType.PRIVATE) {
		    			var item = list[i];
		    			jQuery.ajaxSettings.async = false;
		    			jQuery.post("/action/portal/rcstoken/userInfo",{userId:item.targetId},function(date){
							item.senderUserName = date.name;
							item.senderPortraitUri = date.portrait;
						});
		    			jQuery.ajaxSettings.async = true;
			    		_list.push(item);
		    		}
	...
	var openConversation = function(conversation){
		...
		var callbacks = function(list,hasMsg){
			...
			jQuery.ajaxSettings.async = true;
			console.log(messageList.senderName);
			console.log(messageList.senderPortrait);
			data.messageList = render(templates.imMessage, messageList);
			data.targetName = '用户：'+messageList.senderName;
	...
	var getHisMessage = function(conversationId,timestrap,count,callbacks){
		...
		RongIMLib.RongIMClient.getInstance().getHistoryMessages(conversationType, targetId, timestrap, count, {
		  onSuccess: function(list, hasMsg) {
			  console.log(list);
			  console.log(conversation.messageContent);
	...
	var init = function(config){
		RCS.config = config;
		config.isIM = true;
		myPortrait = config.extraInfo.userInfo.portrait;
		var callbacks = {
