...
if(_a8=="last"){
// modify by zhenghd 20160926 delete pagination last-button
// bb.last=_a9("last");
}else{
if(_a8=="manual"){
...
// 当前页乘以每页显示数量加1小于总数是表示统计总数，分页显示原来逻辑
if(_b6.pageNumber * _b6.pageSize + 1 < _b6.total){
	...
	if(_b6.pageNumber == 1){
		// if fisrt page
		_ba = _ba.replace(/{from}/, 1);
		
		if(_b6.pageNumber * _b6.pageSize < _b6.total){
			_ba = _ba.replace(/{to}/, _b6.pageSize * _b6.pageNumber);
			_ba = _ba.replace(/{total}/, '<a href="javascript:void(0);" class="page-query">'+_b6.pageSize * _b6.pageNumber + '+</a>');
			
		}else{
			_ba = _ba.replace(/{to}/, _b6.total);
			_ba = _ba.replace(/{total}/, _b6.total);
		}
		
	}else{
		// if pre-button, pageSize * pageNumber < total
		if(_b6.pageNumber * _b6.pageSize < _b6.total){
			_ba = _ba.replace(/{from}/, _b6.total == 0 ? 0 : _b6.pageSize * (_b6.pageNumber - 1) + 1);
			_ba = _ba.replace(/{to}/, _b6.pageSize * _b6.pageNumber);
			_ba = _ba.replace(/{total}/,'<a href="javascript:void(0);" class="page-query">'+ _b6.pageSize * _b6.pageNumber + '+</a>');
		...
		// if next-button
			// console.log(_b6.total);
			// console.log(_b6.pageSize);
			// console.log(_b6.pageNumber);
			var p_count = _b6.pageSize * (_b6.pageNumber -1) + 1;
			if(p_count == _b6.total){
				_ba = _ba.replace(/{from}/, _b6.total == 0 ? 0 : _b6.pageSize
						* (_b6.pageNumber - 2) + 1);
				_ba = _ba.replace(/{to}/, _b6.pageSize * (_b6.pageNumber - 1));
				_ba = _ba.replace(/{total}/,'<a href="javascript:void(0);" class="page-query">'+ _b6.pageSize * (_b6.pageNumber - 1) + '+</a>');
			}else{
				_ba = _ba.replace(/{from}/, _b6.total == 0 ? 0 : _b6.pageSize
						* (_b6.pageNumber - 1) + 1);
				if(_b6.pageSize * _b6.pageNumber < _b6.total){
					_ba = _ba.replace(/{to}/, _b6.total - 1);
					_ba = _ba.replace(/{total}/, '<a href="javascript:void(0);" class="page-query">'+_b6.total - 1 + '+</a>');
				}
				else{
					_ba = _ba.replace(/{to}/, _b6.total);
					_ba = _ba.replace(/{total}/,  _b6.total);
...
_4ca.css({width:opts.iconWidth+2+"px",height:tb.height()+"px"});
...
var th=$(this);
var col=$.extend({},$.parser.parseOptions(this,["field","align","halign","order","width","currency","lov","mark",{mvgReadOnly:"boolean",defaultSearch:"boolean",noSearch:"boolean",sortable:"boolean",checkbox:"boolean",resizable:"boolean",fixed:"boolean"},{rowspan:"number",colspan:"number"}]),{title:(th.html()||undefined),hidden:(th.attr("hidden")?true:undefined),readOnlyCol:(th.attr("readOnlyCol")?eval(th.attr("readOnlyCol")):undefined),formatter:(th.attr("formatter")?eval(th.attr("formatter")):undefined),styler:(th.attr("styler")?eval(th.attr("styler")):undefined),sorter:(th.attr("sorter")?eval(th.attr("sorter")):undefined)});
