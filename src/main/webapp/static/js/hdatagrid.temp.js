...
(function($){
	$.fn.hdatagrid = function(options, param){
		...
		return this.each(function(){
            ...
            var that = this;
            $(this).datagrid("getPanel").attr('tabindex', 1).unbind(".datagrid").bind("keydown.datagrid",function(e){
            	
            	if(e.shiftKey == true){
            		$(that).datagrid('options').hdatagrid.options.shiftKey = true;
            	}
            	if(e.ctrlKey == true){
            		return false;
            	}
            	...
            	e.stopPropagation();
            		...
            		if(e.keyCode == 16){ //shift
            			$(that).datagrid('options').hdatagrid.options.shiftKey = false;
                	}
            		console.log("up");
            		console.log(e);
  ...
  //内遮罩延时
		...
		var datagridOpts = $.extend({},this.options,{
		    ...
		    onClickRow:function(index,row){
		    	...
		    	if(options.hdatagrid.options.batchEditFlag == true && options.hdatagrid.options.shiftKey == true){
		    		...
		    		if(row){
		    			var index_1 = $(this).datagrid('getRowIndex', row);
		    		}
		    		...
		    		if(options.hdatagrid.options.status == "Edit_Rows"){
		    			...
		    			for(var i=0; rows && i<rows.length; i++){
		    				var indexSelected = $(this).datagrid('getRowIndex', rows[i]);
		    				if(indexSelected < index_1 || indexSelected >index_2){
		    					$(this).datagrid("unselectRow", indexSelected);
		    				}
		    			}
		    		}else{
		    ...
		    onDblClickRow:function(index,row){
		    	...
		    	if(options.hdatagrid.options.editIndex != null){
		    		...
		    		$(this).datagrid('beginEdit', options.hdatagrid.options.editIndex);
					$(this).datagrid('getSelected').row_status = 'UPDATE';//添加了行状态为 更新 操作
		    	...
		    	var options = $(this).datagrid('options');
		    	var fields = $(this).datagrid('getColumnFields');//datagrid列集合
				for(var i=0;i < fields.length;i++){
					var opts =$(this).datagrid('getColumnOption', fields[i]);//列属性集合
					if(!opts.editor) continue;
					if(!opts.readOnlyCol) continue;
		    ...
		    onSelect:function(index,row){
				...
				//本表在批量修改状态，不刷新子表
				/*if(options.hdatagrid.options.batchEditFlag === true){
					if(options.hdatagrid.options.beginEditSelectedFlag === true){
							//该行只读
	...
	HDataGrid.prototype.initToolBar = function(){
		...
		if(this.options.buttonSearch){
			...
			this.addToolbarTag(-20, '<div id="textSearchBoxDiv" style="display:inline"><input style="width: 200px;" type="text" name="searchValue" class="easyui-searchbox-list-table moreToolbar_notHideFlag" /></div>', this);
			this.addToolbarTag(-20, '<div id="lovSearchBoxDiv" style="display:none"><input style="width: 200px; height:24px" id="lovSearch"  name="searchValue" class="easyui-lov-searchbox-list-table easyui-combobox moreToolbar_notHideFlag" /></div>', this);
			this.addToolbarTag(-20, '<div id="dateSearchBoxDiv" style="display:none"><input style="width: 200px; height:24px" id="dateSearch"  name="searchValue" class="easyui-date-searchbox-list-table easyui-datebox moreToolbar_notHideFlag" /></div>', this);
		...
		if(this.options.buttonNew && top.addFlag == "Y"){
			this.addToolbarTag(20, '<a href="javascript:void(0)" class="easyui-linkbutton-add-list-table moreToolbar_notHideFlag standard-query-toolbar" iconCls="icon-add" plain="true" >新建</a>', this);
		}
		if(this.options.buttonCopy && top.addFlag == "Y"){
			this.addToolbarTag(30, '<a href="javascript:void(0)" class="easyui-linkbutton-copy-list-table moreToolbar_notHideFlag standard-query-toolbar" iconCls="icon-copy" plain="true">复制</a>', this);
		}
		if(this.options.buttonDelete && top.deleteFlag == "Y"){
			this.addToolbarTag(40, '<a href="javascript:void(0)" class="easyui-linkbutton-delete-list-table moreToolbar_notHideFlag standard-query-toolbar" iconCls="icon-remove" plain="true">删除</a>', this);
		}
		...
		this.addToolbarTag(90, '<a href="javascript:void(0)" class="easyui-linkbutton-save-list-table moreToolbar_notHideFlag standard-edit-toolbar" style="display:none" iconCls="icon-save" plain="true">保存</a>', this);
		this.addToolbarTag(100, '<a href="javascript:void(0)" class="easyui-linkbutton-undo-list-table moreToolbar_notHideFlag standard-edit-toolbar" style="display:none" iconCls="icon-undo" plain="true">撤销</a>', this);
		...
		if(this.options.buttonBatchEdit && top.editFlag == "Y" && this.options.updateable){
			...
			this.addToolbarTag(114, '<a href="javascript:void(0)" class="easyui-linkbutton-batchEdit-select-all-list-table moreToolbar_notHideFlag batch-edit-toolbar" style="display:none" iconCls="icon-edit" plain="true" >全选</a>', this);
			...
			this.addToolbarTag(119, '<a href="javascript:void(0)" class="easyui-linkbutton-batchEdit-save-list-table moreToolbar_notHideFlag batch-edit-toolbar" style="display:none" iconCls="icon-save" plain="true" >保存</a>', this);
			this.addToolbarTag(120, '<a href="javascript:void(0)" class="easyui-linkbutton-batchEdit-undo-list-table moreToolbar_notHideFlag batch-edit-toolbar" style="display:none" iconCls="icon-undo" plain="true" >撤销</a>', this);
		}
		...
		if(this.options.buttonMenu){
			if(this.options.buttonBatchEdit && top.editFlag == "Y" && this.options.updateable){
				...
				this.addToolbarTag(9999, str, this,true);
			}
	...
	HDataGrid.prototype.dealEventAndMethod = function(){
		...
		if(options.buttonSearch){
			...
			$(searchInput).combobox('loadData', [{val:'IS NULL', name: '空值', group:'辅助查询'}, {val: 'NOT NULL', name: '非空',group:'辅助查询'}]);
					...
					for(var i=0; i<items.length; i++){
						...
						if(item.selected){
							if(item.lov){
								...
								var comboData = getLovArray(item.lov);
		...
		if(options.buttonCombineSearch){
			...
			$(buttonCombineSearch).linkbutton({
				onClick:function(){
		...
		if(options.buttonNew){
			var buttonNew = $(datagridPanel).find('.easyui-linkbutton-add-list-table');
			$(buttonNew).linkbutton({
				onClick:function(){
		...
		if(options.buttonDelete){
			var buttonDelete = $(datagridPanel).find('.easyui-linkbutton-delete-list-table');
			$(buttonDelete).linkbutton({
				onClick:function(){
		...
		if(options.buttonCopy){
			var buttonCopy = $(datagridPanel).find('.easyui-linkbutton-copy-list-table');
			$(buttonCopy).linkbutton({
				onClick:function(){
					that.copyOneRecord();
		...
		if(options.buttonExport){
			var buttonExport = $(datagridPanel).find('.easyui-linkbutton-export-list-table');
			$(buttonExport).linkbutton({
				onClick:function(){
					that.exportGridData();
				}
			});
		...
		if(options.buttonXmlExport){
			var buttonXmlExport = $(datagridPanel).find('.easyui-linkbutton-xml-export-list-table');
			$(buttonXmlExport).linkbutton({
				onClick:function(){
		...
		if(options.buttonXmlImport){
			var buttonXmlImport = $(datagridPanel).find('.easyui-linkbutton-xml-import-list-table');
			$(buttonXmlImport).linkbutton({
				onClick:function(){
		...
		if(that.options.buttonBatchEdit){
			if(true){
				...
				$(buttonEditMulRows).click(function(){
					//设置批量修改标记，选中行时不刷新子表，不可双击修改，父表切换时本表不刷新
					that.options.batchEditFlag = true;
					//that.options.beginEditSelectedFlag = true;
					that.options.status = 'Edit_Rows';
					/**工具栏按钮切换（父子表）*/
					that.changeToolBarStatus();
					//$(datagridPanel).find('.easyui-linkbutton-batchEdit-uniform-selected-list-table').css("display", "none");
					//隐藏子表按钮
					var subGridIds = that.options.subGridIds;
					if( subGridIds.length > 0 ){
						for(var i=0; i<subGridIds.length; i++){
							var subGridId = subGridIds[i];
							if($('#'+subGridId).length){
								$('#'+subGridId).datagrid("options").hdatagrid.hideToolBarButton();
							}
						}
					}
					/**表格变为可多选 */
				...
				//统一编辑多行按钮
				var buttonEditMulRowsUniform = $(datagridPanel).find('.easyui-hdatagrid-splitbutton-menu-editMulRows-uniform-'+this.options.id);
				$(buttonEditMulRowsUniform).click(function(){
					//设置批量修改标记，选中行时不刷新子表，不可双击修改，父表切换时本表不刷新
					that.options.batchEditFlag = true;
					...
					if( subGridIds.length > 0 ){
						for(var i=0; i<subGridIds.length; i++){//给子表构造查询参数
							var subGridId = subGridIds[i];
							if($('#'+subGridId).length){
								$('#'+subGridId).datagrid("options").hdatagrid.hideToolBarButton();//隐藏子表按钮
							}
						}
					}
					/**表格变为可多选 */
			...
			//批量修改撤销按钮
			if(true){
				var $buttonBatchEditUndo = $(datagridPanel).find('.easyui-linkbutton-batchEdit-undo-list-table');
				$($buttonBatchEditUndo).linkbutton({
					onClick:function(){
						//清除多选
						//取消批量修改标记
			...
			//批量修改-保存按钮
			if(true){
				var buttonBatchEditSave = $(datagridPanel).find('.easyui-linkbutton-batchEdit-save-list-table');
				$(buttonBatchEditSave).linkbutton({
					onClick:function(){
						//获取修改后数据并保存
			...
			//批量修改全选按钮
			if(true){
				var buttonBatchEditSelectAll= $(datagridPanel).find('.easyui-linkbutton-batchEdit-select-all-list-table');
				$(buttonBatchEditSelectAll).linkbutton({
					onClick:function(){
						//选中当前页全部记录
			...
			//批量修改 统一值修改(选中)按钮
			if(true){
				var $buttonBatchEditSelectAllUniformEdit= $(datagridPanel).find('.easyui-linkbutton-batchEdit-uniform-selected-list-table');
				$($buttonBatchEditSelectAllUniformEdit).linkbutton({
					onClick:function(){
						//获取当前页所有选中数据
						var rows = $(that.el).datagrid("getSelections");
						if(!rows || !rows.length){
						...
						var confirm = function(){
							...
							$.messager.confirm("修改确认", "把选中的行数据统一做以下修改：<br>"+confirmMsg, function(r){
								if(r){
									...
									$dialog.dialog("destroy");
									$('.easyui-linkbutton-batchEdit-save-list-table', $(datagridPanel)).trigger("click");
								}else{
		...
		//详细信息按钮
		if(true){
			var buttonHelp = $('.easyui-hdatagrid-splitbutton-menu-help'+this.options.id);//下拉菜单生成的html元素不在datagrid的Panel上
		...
		if(options.menuButtonCount){
			var buttonCount = $('.easyui-hdatagrid-splitbutton-menu-count'+this.options.id);//下拉菜单生成的html元素不在datagrid的Panel上
			$(buttonCount).click(function(e){
				that.count();
			});
		}
	...
	HDataGrid.prototype.count = function(){
		...
		if(options.menuButtonCount){
			if(options.countUrl == '' || options.countUrl == null){
				//拼接出默认url
				//options.countUrl = that.getControllerUrl($(that.el).datagrid('options').url) + '/countByExample';
	...
	HDataGrid.prototype.onOperationSuccess=function(that, data, unChange){
		...
		if(that.options.continueNewFlag == true){//继续新建数据
			var $datagridPanel = $(that.el).datagrid("getPanel");
			$('.easyui-linkbutton-add-list-table', $datagridPanel).trigger("click");
			...
			var $datagridPanel = $(that.el).datagrid("getPanel");
			$('.easyui-linkbutton-copy-list-table', $datagridPanel).trigger("click");
		}else{
			that.reInitHDataGridData();
	...
	HDataGrid.prototype.undoOperationH=function(){
		...
		var index = $(this.el).datagrid('rejectChanges');
	...
	HDataGrid.prototype.exportGridData=function(){
		...
		$.post(datagridOptions.url+'/exportData',formatRequest(queryParams),function(data){
			if(data.success){
	 ...
	 * 工具栏按钮切换到批量修改状态
	 */
	/*HDataGrid.prototype.changeToolBarUpdateStatus=function(){
		var options = this.options;
		var datagridPanel = $(this.el).datagrid("getPanel");
		$('.standard-query-toolbar',$(datagridPanel)).css("display", "none");
		$('.standard-edit-toolbar',$(datagridPanel)).css("display", "none");
		...
		$('.moreToolbar',$(datagridPanel)).css("display", "none");
		//$('.toolbar-menu-more', $(datagridPanel)).css("display", "none");
	}*/
	/**
	 * 完成或取消操作
	 */
	/*HDataGrid.prototype.finishedSaveOrUndoOperation=function(){
		var options = this.options;
		var datagridPanel = $(this.el).datagrid("getPanel");
	 ...
	 * 编辑时工具栏状态切换
	 */
	/*HDataGrid.prototype.toSaveOrUndoOperation=function(){
		var datagridPanel = $(this.el).datagrid("getPanel");
	...
	HDataGrid.prototype.addToolbarTag=function(seq, tag, that, isPullDown){
		...
		if(isPullDown){
			...
			/*if(tag != null && tag.indexOf("moreToolbar_notHideFlag") < 0 ){
				//tag = '<div class="moreToolbar">' + tag + '</div>';//为外部标签加上moreToolbar类，操作标签隐藏
			} else {
				tag = '<div>' + tag + '</div>';
			}
			//tag = '<div>' + tag + '</div>';*/
		...
		for(var i = 0; i < columns.length; i++){
			...
			if(columns[i].currency && !columns[i].formatter){//为设置了currency属性的列添加货币符号(以formatter的方式实现)
				...
				case "USD"://美元
					...
					break;
				case "HKD"://港币
					columns[i].formatter = function(value,rowData,rowIndex){return value!=null ? ('HK$' + parseFloat(value).toLocaleString()) : value};
					break;
				case "EUR"://欧元
					columns[i].formatter = function(value,rowData,rowIndex){return value!=null ? ('€' + parseFloat(value).toLocaleString()) : value};
					break;
	...
	$.fn.hdatagrid.methods = {
		...
		updateCurrentRow:function(target,currentRow){
			...
			/*if(options.batchEditFlag === true){
				*//** 非逐行修改 *//*
				if(options.beginEditSelectedFlag === false){
