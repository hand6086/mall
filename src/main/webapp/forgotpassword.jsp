...
<body>
	...
	<form id="form">
				...
				<fieldset class="fieldset">
					<div class="enter">
						<label id="userid" class="userid" style="display: none"></label>
						<input type="text" name="username" id="username" class="entry" value="请输入用户名"/>
			...
			$(function() {
				...
				$("#sendSMSCode").click(function() {
					...
					if(!p_phone){
						...
						if($('#form').valid()){
							$.post('<%=ctxPath%>/action/portal/user/checkUserInfo',
							{
							...
							function(data){
							    if (data.success) {
                                    ...
                                    $.post('<%=ctxPath%>/action/portal/sms/getCode',{
                                        contactPhone:p_phone
                                    },
									function(data){
										if(data.success){
											...
											interval = setInterval(function() {
												$('#sendSMSCode').val(num + 'S 后再次发送');
												num--;
												if (num < 0) {
													$('#phonenumber').attr("disabled", false);
													$('#sendSMSCode').removeAttr("disabled");
													$('#sendSMSCode').attr({'class':'code'});
													$('#sendSMSCode').val('获取手机验证码');
				...
				$("#confirmCode").click(function() {
					...
					if(!p_code && !p_phone){
						...
						if($('#form').valid()){
							$.post('<%=ctxPath%>/action/portal/sms/resetPwdVerifyCode',
							{
								VerifyCode : p_code,
								contactPhone:p_phone,
                                username:p_username
							...
							function(data) {
								if (data.success) {
									clearInterval(interval);
									$('body').load("<%=ctxPath%>/resetpassword.jsp",{"username":p_username,"contactPhone":p_phone});
								} else {
									$('#showError').show();
									$('#showError').text("验证码错误");
								}
				...
				$("#username").click(function(){
					if($("#username").val().indexOf("请输入用户名")>=0){
						$("#username").val(null);
